...
export const VENDORS = {
  icecream: {
    name: 'Bà Năm', x: 1028, y: 1222, look: GRANNY('#f4a9b8', '#fffaf0'),
  ...
  sugarcane: {
    name: 'Bà Hai', x: 1062, y: 1474, look: GRANNY('#b9d7a0', '#f7de8c', '#c9c2c6', 'nonla'),
  ...
  banhmi: {
    name: 'Bà Út', x: 782, y: 1220, look: GRANNY('#f7de8c', '#e9c9a2', '#b9b3ba'),
    hi: [['Bánh mì nóng giòn! Only a few coins.', 'Bánh mì nóng giòn đây! Rẻ lắm con.'], ['You look hungry. Bà Út can fix that.', 'Nhìn con đói quá. Để Bà Út lo.']],
    menu: [{ id: 'banh_mi_bite', en: 'Crispy bánh mì', vi: 'Bánh mì giòn', price: 6 }],
    act: 'eat', done: [['Crunchy! Delicious!', 'Giòn rụm! Ngon quá!'], ['That hit the spot.', 'Đã cái bụng ghê.']],
  },
};
...
export function spawnVendors(island) {
  for (const [id, v] of Object.entries(VENDORS)) {
    const a = new Actor({ kind: 'human', look: v.look, name: v.name, x: v.x, y: v.y, data: { vendor: id } });
...
export async function talkToVendor(a) {
  ...
  try {
    ...
    if (!canAfford(item.price)) { sfx('error'); await say(a, T('Short on coins? Next time, cháu.', 'Hết tiền hả? Lần sau nha cháu.')); return; }
...
export async function buyFromVendor(a) {
  ...
  if (item) await consume(VENDORS[a.data.vendor], item);
}
